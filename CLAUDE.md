...
1. **ネタ収集（自動）**: `fetch-news.yml`（30分毎・RSS27本 → `output/news_feed.json`）＋ `fetch-slack.yml`（30分毎・Slack #x-influencer-watch → `output/slack_buzz.json`=建設系 / `output/ai_buzz.json`=AI系）。X側の監視はローカルMacの `AI-work/scripts/x_watcher.py`（2h毎・建設42＋AI16アカウント・AI系は「🌐 AI」タグ付き）
...
## 補助ワークフロー
- `fetch-news.yml`: 30分毎にRSS → `output/news_feed.json`（category: ai_global/ai_japan/construction_global/construction_japan・`ai_related`・48h窓）
...
- 型: 【タグ】固有名詞＋結論 → 本音1行 → ・箇条書き3〜5（数字はここ） → 建設への含意1〜2行。**段落ごとに空行**・100〜200字・**画像必須**（記事スクショ）・**■補足の自己リプに出典URL**（本文URLは公式原文へ誘導したい時に使用可）
