# ルーティン共通仕様 v4.1（x-post-morning / noon / evening）

戦略・文体・NG表現の正本は `docs/x-strategist.md`（v4.1）。この文書は**3ルーティン共通の手順仕様**だけを持つ。矛盾したら x-strategist.md が勝つ。
...
- 休載するのは**WebSearchで裏取り可能なネタが本当にゼロの時だけ**。「新情報が弱い」「マンネリ気味」程度では休載しない（最終ゲートは新田さんのリライト。判断材料を届けることが仕事）
- 2026-07-31朝の「新情報/鮮度不可で休載」は過剰判定だった。ああいう日でも最良の1本を下書きとして出す
