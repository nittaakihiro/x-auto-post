...
| evening（19:00 JST） | 20:00 | ③ 建設ニュース | **国内外の建設×AI優先**（`news_feed.json` の construction_japan／construction_global で `ai_related=true`。Googleニュース由来の記事は `url` がリダイレクトなので WebSearch でタイトル検索して一次ソースを開く）。**AI以外の建設時事は週1〜2本まで**（直近7日の post_queue に非AIの建設時事が2本あれば使わない。使うのは倒産・法改正・災害・大型受注など業界全体の大ネタだけ）。無ければ `slack_buzz.json`／`ai_buzz.json` のバズ投稿を引用RT（`post_type='quote_rt'`＋`quote_tweet_id`）。それも無ければ型パクリ |
...
- [ ] です・ます基調・本音1行あり・絵文字なし・ハッシュタグなし・出典導線あり
- [ ] 建設への含意1〜2行（柱②。柱①は自然な場合のみ）
...
## v5.1 実行補足（2026-09-09）
- news_feed.jsonに加えて、RSSにない国内外の公式発表・建設AI導入事例をWeb検索で補う。鮮度不足の記事は最新と称しない。
- 各枠の生成時は過去14日のキューと、当日の実投稿を確認し重複を避ける。
- dashboard.original_postに今回の下書き、engage_cardsに今回の候補を入れる。推奨1〜2件を先頭に。Slackは今回のoriginal_postだけ配信する。
- fact_checkに発表日時・確認日時・提供条件・未検証事項を残す。未確認の一人称体験を投稿本文へ混ぜない。
- 一般AIを建設と無理につなげる必要はない。本文内リンクの有無が表示回数の原因とは断定しない。
