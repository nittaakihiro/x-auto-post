...
2. 朝＝一般AI、昼＝建設AI/実演、夕＝AI/建設の引用、夜＝強い海外続報がある時だけ追加。国内外を検索し候補5件を比較。各候補は前段（前バージョン・従来のやり方）と、影響を受ける現場の作業まで調べる。競合サービスの紹介は戦略の除外ルールに従い、本文・補足・引用・リプ・RT候補から外す。
3. 戦略の5観点各0〜2点で評価。一次本文を開いて利用条件と日付を確認。採用ネタに編集上の理由を一文でつける。スコアは主観的な編集基準でありバズ予測ではない。
4. 速報・実演・比較・海外事例・引用から形式を選ぶ。ニュース系は要約で終えず、どういう発展・進化か（前段との差）と現場レイヤーの作業がどう変わるかを本文に入れる。海外ニュースは日本に届く段階まで書く。冒頭を2案作り、具体性が高く誇張のない方を採用。段落数、感想、箇条書き、建設への接続を機械的に強制しない。
5. 完成文＋出典＋素材URLを作り、セルフレビュー。原文と数字・提供条件の一致、未確認体験の不在、重複、同じ型の連続をチェック。発展の説明と現場の変化が入っているか、確認していない効果を断定していないかも見る。速報タグは鮮度がある時だけ。
...
7. 追加したentryにeditorial={pillar, format, scores, selection_reason, source_url, published_at, checked_at, availability, unverified, hook_alternative}を追加し、save_queueでキューを保存。既存フィールドは壊さない。ニュース系はdevelopment（前段との差＝どういう発展か）とfield_change（どの作業がどう変わるか）もeditorialに残す。この2つはvalidate_editorial.pyの必須項目にしていないため、欠けても検証は通る。本文に書けていれば検証は通るという理由で省略しない。
