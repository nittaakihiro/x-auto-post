...
- **動画の埋め込み（チャエン式・2026-09-09）**: 元になった海外投稿に動画がある時は、その動画のURL `https://x.com/<user>/status/<id>/video/1` を**本文の最後の行**に貼る。Xがそれを動画プレーヤーとして埋め込み「投稿者: @元アカウント」のクレジットが自動で付く（リンクカードにならないのでペナルティ対象外。チャエンの【超速報】Images 2.5 投稿は OpenAI の動画をこの方式で埋め込んでいる）。ルーティンは `video_url` に渡し、新田さんは投稿時に末尾へ貼る。動画がある時は画像不要
